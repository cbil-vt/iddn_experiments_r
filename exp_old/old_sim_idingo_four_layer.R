...
for (n in 1:n_rep) {
  ...
  fit <- idingo(dat_miRNA, dat2 = dat_protein, dat3=dat_mRNA,
                x = dat_class, plats = c("miRNA", "Protein", "mRNA"),
                diff.score = TRUE, B = 20, cores = 10)

  genepair = fit$genepair
  diffscore = fit$diff.score

  molname_protein = paste0("Protein", "||", colnames(dat_protein))
  molname_mRNA = paste0("mRNA", "||", colnames(dat_mRNA))
  molname_miRNA = paste0("miRNA", "||", colnames(dat_miRNA))

  # The dataset has order protein, mRNA, and miRNA+lncRNAs
  molname = c(molname_protein, molname_mRNA, molname_miRNA)
