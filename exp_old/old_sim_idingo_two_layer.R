...
sim_name = "sim3_tf_mrna_n-hub_2_n-node_144_hill_1.0_sigma_bi_2_1_2_odd_even_ratio_0.25_n_10000"
...
for (n in 1:n_rep) {
  ...
  dat_layer1 = dat12_sel[,1:n1]
  dat_layer2 = dat12_sel[,(n1+1):(n1+n2)]
  colnames(dat_layer1) = paste0("mol_", 1:n1)
  colnames(dat_layer2) = paste0("mol_", (n1+1):(n1+n2))

  fit <- idingo(dat_layer1, dat2 = dat_layer2, x = dat_class, plats = plats,
                diff.score = TRUE, B = 20, cores = 10)

  genepair = fit$genepair
  diffscore = fit$diff.score

  molname1 = paste0(plats[[1]], "||", colnames(dat_layer1))
  molname2 = paste0(plats[[2]], "||", colnames(dat_layer2))
  molname = c(molname1, molname2)
